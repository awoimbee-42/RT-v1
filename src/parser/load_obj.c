...
/*   Updated: 2019/03/02 04:28:47 by awoimbee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Object(std::vector<int> position = { 0, 0, 0 }, std::vector<int> color = { 255,255,255 }, std::string path) {
		std::ifstream modelFile;
		modelFile.open(path);
		std::string readLine;
		int fileOindex = 1;
		...
		if (modelFile.is_open()) {
			...
			while (!modelFile.eof()) {
				...
				if (readLine[0] == 'o' && readLine[1] == ' ' && oNumber < fileOindex) {
				...
				else if (readLine[0] == 'v' && readLine[1] == 't' && (oNumber == fileOindex || oNumber == 0)) {
					std::vector<float> texCoord(3);
					char trash[2];
					iss >> trash >> texCoord[0] >> texCoord[1];
					...
					for (std::string& face : faceFull) { // v/vt/vn
						...
						faceVerts.push_back(std::stoi(temp) - 1);
						std::getline(issFace, temp, '/'); // it's texture = trash for now
						faceTex.push_back(std::stoi(temp) - 1);
						std::getline(issFace, temp, '/');
						faceNorms.push_back(std::stoi(temp) - 1);
					}

					this->faces.push_back({ faceVerts, faceNorms, faceTex });
				}
				else if (readLine[0] == 'g' && readLine[1] == ' ') {
					//change color or something
				}
